...
[input_1, after_cypre_1] = transmitter_ofdm(pn_order, init_state, length_input, pilot_locations, symbol_size, trellis, st_int, moduation_type, pilot_value, fft_size, cp_size);
[input_2, after_cypre_2] = transmitter_ofdm(15, init_state_2, length_input, pilot_locations, symbol_size, trellis, st_int, moduation_type, pilot_value, fft_size, cp_size);


%s/p
out_signal_trans = after_cypre_1(:);

H_mimo = (randn(numRx, numTx) + 1i*randn(numRx, numTx)) / sqrt(2);
txSignal = [after_cypre_1(:).'; after_cypre_2(:).'];
...
after_channel_awgn = awgn(H_mimo*txSignal,SNR,"measured");
%after_multipath = con
test_signal = after_channel_awgn;
...
%reciever
%s/p
after_channel_awgn = after_channel_awgn(1:2,1:length(out_signal_trans));

%mimo decoding 
H_inv = pinv(H_mimo);  
after_channel_awgn_all = H_inv * after_channel_awgn;%reshape(after_channel_awgn, numRx, fft_size * symbol_size);

after_channel_user1 = after_channel_awgn_all(1,:);

parallel_wrong_freq = reshape(after_channel_user1,fft_size+cp_size,[]);
...
after_channel_user1 = reshape(after_channel_user1,fft_size+cp_size,[]);

%phase_est = after_channel_awgn()

%remove cp
without_cp = after_channel_user1(cp_size+1:end,:);
...
biterr(decodedData,input_1)/length(input_1)
%test_scram = input== final_sig


function [input, after_cypre] = transmitter_ofdm(pn_order, init_state, length_input, pilot_locations, symbol_size, trellis, st_int, moduation_type, pilot_value, fft_size, cp_size)
%pn sequence genertor
input = pn_seq_gen(pn_order,init_state,(length_input-2*length(pilot_locations))*symbol_size);
